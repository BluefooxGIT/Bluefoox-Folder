...
if ( isset( $_GET[ 'm' ] ) ) {
  $funcion_mover = $_GET[ 'm' ];
  if ( $funcion_mover == 's' ) {
    $nombre_archivo = $_POST[ 'nombre-nuevo-archivo' ];
    $directorio_mover_origen = $_POST[ 'directorio-copia-origen' ];
    $directorio_mover_destino = $_POST[ 'directorio-copia-destino' ] . $nombre_archivo;
    $uid_archivo = $_POST[ 'input-id-renombrar' ];
    if ( is_file( $directorio_mover_origen ) ) {
      echo '<script>$("#id-div-carga").show();</script>';
      copy( $directorio_mover_origen, $directorio_mover_destino );
      $sql_mover = $mysqli->query( "SELECT * FROM bluefoox_archivos WHERE uid = '$uid_archivo'" );
      $row_mover = mysqli_fetch_array( $sql_mover );
      $archivos_url = $_POST[ 'directorio-copia-registro' ];
      $archivos_nombre = $row_mover[ 'archivos_nombre' ];
      $archivos_extension = $row_mover[ 'archivos_extension' ];
      $archivos_tipo = $row_mover[ 'archivos_tipo' ];
      ...
      $archivos_usuario = $row_mover[ 'archivos_usuario' ];
      ...
      $archivo_eliminar = '../' . $row_mover[ 'archivos_url' ] . '/' . $row_mover[ 'archivos_nombre' ];
  ...
  if ( $url_retorno != '' ) {
    ...
    echo '<script> function retorno() { $("#id-input-url-raiz").val("' . $url_retorno . '"); lista_navegador_mover(); }</script>';
  }
  if ( isset( $row_registros ) ) {
    do {
      ...
      echo '<script> function navegador_carpetas_' . $uid_carpeta . '() { var carpeta_actual_' . $uid_carpeta . ' = $("#id-input-url-carpeta-actual-' . $uid_carpeta . '").val(); var carpeta_destino_' . $uid_carpeta . ' = $("#id-input-url-carpeta-destino-' . $uid_carpeta . '").val(); var url_destino_' . $uid_carpeta . ' = carpeta_actual_' . $uid_carpeta . ' + "/" + carpeta_destino_' . $uid_carpeta . '; $("#id-input-url-raiz").val(url_destino_' . $uid_carpeta . '); lista_navegador_mover(); } </script>';
  ...
  if ( is_file( $directorio_destino_archivo ) ) {
    echo '<script>$("#id-span-mueve").hide();</script>';
  } else {
    echo '<script>$("#id-span-mueve").show();</script>';
  }
}
?>
