<div id="id-div-contenedor-lista-archivos" class="<?php if(!isset($row_registros)) { echo 'div-oculto'; } ?>">
  ...
  <form action="" method="post" enctype="multipart/form-data" id="id-form-eliminar-archivos">
    ...
    <?php $uid_archivo_temp = $row_registros['uid']; if ($uid_archivo_temp == ' ') { $uid_archivo = 'null'; } else { $uid_archivo = $uid_archivo_temp; } ?>
    <div id="id-archivo-detalles-<?php echo $uid_archivo; ?>" class="div-archivo-detalles hvr-forward">
      <div class="div-checkbox-eliminar">
        <label class="label-checkbox-eliminar<?php if($row_registros['archivos_tipo'] == 'c') { echo '-oculto'; } ?>">
          <input type="checkbox" id="id-checkbox-eliminar" onClick="boton_eliminar()" class="checkbox-eliminar" name="checkbox-eliminar[]" value="<?php echo $row_registros["uid"]; ?>">
          <span class="span-checkmark-eliminar"></span></label>
      </div>
      <?php
      if ( $row_registros[ 'archivos_tipo' ] == 'c' ) {
        $url_carpeta_temp = $archivos_url . '/' . $row_registros[ 'archivos_nombre' ];
        $url_carpeta = '?r=' . urlencode( base64_encode( $url_carpeta_temp ) );
        echo '<a class="a-archivo" href="' . $url_carpeta . '">';
      } else if ( $row_registros[ 'archivos_tipo' ] == 'a' ) {
        ...
        if ( $archivos_extension == 'jpg' || $archivos_extension == 'jpeg' || $archivos_extension == 'png' || $archivos_extension == 'mp4' || $archivos_extension == 'pdf' ) {
          echo '<a class="span-titulo-vista-previa" onClick="vista_previa_' . $row_registros[ "uid" ] . '()">';
      ...
      <div class="div-archivo">
        <div class="div-titulo-archivo">
          ...
          <div class="f">
            <div class="s">
              <div class="<?php $rango_nombre_archivo = strlen($row_registros['archivos_nombre']); if($rango_nombre_archivo > '30') { echo 't'; } else { echo 'tt'; } ?>">
                <?php if($row_registros['archivos_tipo'] == 'c') { if(isset($row_registros['archivos_nombre'])) { echo $row_registros['archivos_nombre']; }} else if($row_registros['archivos_tipo'] == 'a') { if(isset($row_registros['archivos_nombre'])) { echo $row_registros['archivos_nombre']; }}?>
              </div>
            </div>
          </div>
        </div>
        <?php if($row_registros['archivos_tipo'] == 'c') { $url_carpeta_temp = $archivos_url.'/'.$row_registros['archivos_nombre']; $url_carpeta = '?r='.urlencode( base64_encode( $url_carpeta_temp ) ); echo '</a>'; } else if($row_registros['archivos_tipo'] == 'a') { echo '</a>'; }?>
        <input type="hidden" id="id-input-archivos-nombre-<?php echo $uid_archivo; ?>" name="archivos_nombre" value="<?php if(isset($row_registros['archivos_nombre'])) { $nombre_archivo_fix = mb_strimwidth($row_registros['archivos_nombre'], 0, 24, "... "); echo preg_replace('/\\.[^.\\s]{3,4}$/', '', $nombre_archivo_fix); } ?>">
        <input type="hidden" id="id-input-archivos-extension-<?php echo $uid_archivo; ?>" name="archivos_extension" value="<?php echo $row_registros['archivos_extension']; ?>">
        <input type="hidden" id="id-input-archivos-tamano-<?php echo $uid_archivo; ?>" name="archivos_tamano" value="<?php if($row_registros['archivos_tipo'] == 'a') { $archivo_tamano_temp = filesize(''.$row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']); if ($archivo_tamano_temp >= 1073741824) {
        ...
        <input type="hidden" id="id-input-archivos-modificacion-<?php echo $uid_archivo; ?>" name="archivos_modificacion" value="<?php echo $row_registros['archivos_modificacion']; ?>">
      </div>
      ...
      <div id="id-div-subherramientas-archivos-<?php echo $uid_archivo; ?>" class="div-subherramientas-archivos">
        <div class="<?php if($row_registros['archivos_tipo'] == 'c') { echo 'div-oculto'; } else { echo 'div-herramientas-archivos'; } ?>" title="Copiar" onClick="copiar_<?php echo $uid_archivo; ?>();lista_navegador_copiar()">
        ...
        <div class="<?php if($row_registros['archivos_tipo'] == 'c') { echo 'div-oculto'; } else { echo 'div-herramientas-archivos'; } ?>" title="Mover" onClick="mover_<?php echo $uid_archivo; ?>();lista_navegador_mover()">
          <svg class="svg-herramientas-archivos" viewBox="0 0 24 24">
        ...
        <div class="<?php if($row_registros['archivos_tipo'] == 'c') { echo 'div-oculto'; } else { echo 'div-herramientas-archivos'; } ?>" title="Descargar">
          <svg class="svg-herramientas-archivos" viewBox="0 0 24 24">
        ...
        <div class="<?php if($row_registros['archivos_tipo'] == 'c') { echo 'div-oculto'; } else { echo 'div-herramientas-archivos'; } ?>" title="Eliminar">
          <svg class="svg-herramientas-archivos" viewBox="0 0 24 24">
    ...
    <input type="hidden" id="id-nombre-archivo-<?php echo $uid_archivo; ?>" value="<?php $nombre_archivo = pathinfo('../'.$row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']); echo $nombre_archivo['filename']; ?>">
    <input type="hidden" id="id-tipo-archivo-<?php echo $uid_archivo; ?>" value="<?php echo $row_registros['archivos_tipo']; ?>">
    <input type="hidden" id="id-extension-archivo-<?php echo $uid_archivo; ?>" value="<?php echo $row_registros['archivos_extension']; ?>">
    ...
    function copiar_<?php echo $uid_archivo; ?>() {
      ...
      var directorio_copia_archivo_original = '../<?php echo $row_registros['archivos_url']; ?>/<?php echo $row_registros['archivos_nombre']; ?>';
      var uid_copiar = '<?php echo $uid_archivo; ?>';
      $('#id-input-directorio-copia-origen').val(directorio_copia_archivo_original);
      $('#id-input-nuevo-nombre').val('<?php echo $row_registros['archivos_nombre']; ?>');
    ...
    function mover_<?php echo $uid_archivo; ?>() {
      ...
      var directorio_mover_archivo_original = '../<?php echo $row_registros['archivos_url']; ?>/<?php echo $row_registros['archivos_nombre']; ?>';
...
function vista_previa_<?php echo $uid_archivo; ?>() {
  ...
  if (archivo_extension_<?php echo $uid_archivo; ?> == 'jpg' || archivo_extension_<?php echo $uid_archivo; ?> == 'jpeg' || archivo_extension_<?php echo $uid_archivo; ?> == 'png') {
    $('#id-img-vista-previa').attr('src', '<?php echo '../'.$row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']; ?>');
    <?php $tipo_archivo = $row_registros[ 'archivos_extension' ]; if( $tipo_archivo == 'jpg') { list($ancho_img, $alto_img) = getimagesize($row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']); } ?>
    ...
    $('#id-pdf-vista-previa').hide();
    $("#id-audio-vista-previa").attr('src', '<?php echo '../'.$row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']; ?>');
    $('.div-alerta').css('width', '60%');
    ...
    $('#id-pdf-vista-previa').hide();
    $("#id-video-vista-previa").attr('src', '<?php echo '../'.$row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']; ?>');
    ...
    $("#id-pdf-vista-previa").attr('src', '<?php echo '../'.$row_registros['archivos_url'].'/'.$row_registros['archivos_nombre']; ?>');
  ...
  $('#id-video-vista-previa').get(0).currentTime = 0;
    ...
    <?php } while ($row_registros = mysqli_fetch_assoc($sql_registros)); ?>
