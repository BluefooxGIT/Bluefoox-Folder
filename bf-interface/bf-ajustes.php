...
<div class="div-contenedor-principal">
  ...
  <div class="div-contenido-ajuste">
    ...
    <div class="div-usuario-ajuste">
      <label class="label-titulos-ajuste">Versión 1.0 est&aacute; actualizado.</label>
      <div id="id-div-actualizacion" class="<?php $sha1_github = "https://github.com/BluefooxGIT/Bluefoox-Folder/archive/refs/heads/main.zip"; $sha1_github_nuevo = sha1_file($sha1_github); $sha1_actual = $row_root[ 'configuracion_sha1_actualizacion' ]; if($sha1_github_nuevo != $sha1_actual) { echo 'div-actualizacion'; $github_actualizacion = $sha1_github; $github_actualizacion_nuevo = $_SERVER['DOCUMENT_ROOT'] . 'update_folder.zip'; if ( copy($github_actualizacion, $github_actualizacion_nuevo) ) { $zip = new ZipArchive; $res = $zip->open($github_actualizacion_nuevo); if ($res === TRUE) { $zip->extractTo(getcwd()); $zip->close(); function custom_copy($src, $dst) { $dir = opendir($src); @mkdir($dst); foreach (scandir($src) as $file) { if (( $file != '.' ) && ( $file != '..' )) { if ( is_dir($src . '/' . $file) ) { custom_copy($src . '/' . $file, $dst . '/' . $file); } else { copy($src . '/' . $file, $dst . '/' . $file); } } } closedir($dir); } $src = "Bluefoox-Folder-main"; $dst = getcwd(); custom_copy($src, $dst); } } } else { echo 'div-oculto'; } ?>">Existe una actualizaci&oacute;n, aplicar ahora.</div>
    </div>
  </div>
</div>
<script>
