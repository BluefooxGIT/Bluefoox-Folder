...
<div class="div-contenedor-principal">
  ...
  <div class="div-contenido-ajuste">
    ...
    <div class="div-usuario-ajuste">
      ...
      <?php $sha1_github = "https://github.com/BluefooxGIT/Bluefoox-Folder/archive/refs/heads/main.zip"; $sha1_github_nuevo = sha1_file($sha1_github); $sha1_actual = $row_root[ 'configuracion_sha1_actualizacion' ]; if($sha1_github_nuevo != $sha1_actual) { $github_actualizacion = $sha1_github; $github_actualizacion_nuevo = $_SERVER['DOCUMENT_ROOT'] . 'update_folder.zip'; if ( copy($github_actualizacion, $github_actualizacion_nuevo) ) { $zip = new ZipArchive; $res = $zip->open($github_actualizacion_nuevo); if ($res === TRUE) { $zip->extractTo(getcwd()); $zip->close(); function custom_copy($src, $dst) { $dir = opendir($src); @mkdir($dst); foreach (scandir($src) as $file) { if (( $file != '.' ) && ( $file != '..' )) { if ( is_dir($src . '/' . $file) ) { custom_copy($src . '/' . $file, $dst . '/' . $file); } else { copy($src . '/' . $file, $dst . '/' . $file); } } } closedir($dir); } $src = "Bluefoox-Folder-main"; $dst = getcwd(); custom_copy($src, $dst); mysqli_query( $mysqli, "UPDATE bluefoox_configuracion SET configuracion_sha1_actualizacion = '$sha1_github_nuevo' WHERE usuarios_id = '$usuarios_id'" ); } } } ?>
    </div>
  </div>
</div>
<script>
